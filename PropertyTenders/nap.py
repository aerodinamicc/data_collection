...
def main():
    records = pd.read_csv('nap_records.csv', sep='\t') if os.path.exists('nap_records.csv') else pd.DataFrame(columns=['price', 'type', 'link', 'desc', 'place', 'period_end', 'announcement', 'bidding', 'page_id'])
    unsuc = pd.read_csv('nap_unsuccessful.csv', sep='\t') if os.path.exists('nap_unsuccessful.csv') else pd.DataFrame()

    rec, failed = scrape_nra(records)
    rec = pd.concat([rec, records])
    failed = pd.concat([failed, unsuc])
    rec.to_csv('nap_records.csv', sep='\t', index=False)
    failed.to_csv('nap_unsuccessful.csv', sep='\t', index=False)
