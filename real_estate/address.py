...
def gather_new_articles(current_date):
    ...
    with open('C:/Users/shadow/Downloads/address_result_sale.html', 'r', encoding='utf-8') as f:
        resp_sale = f.read()

    with open('C:/Users/shadow/Downloads/address_result_rent.html', 'r', encoding='utf-8') as f:
        resp_rent = f.read()
    ...
    offers = offers[['id', 'labels', 'type', 'is_for_sale', 'place', 'city', 'currency', 'price', 'broker_info', 'description', 'link']]
    offers['is_for_sale'] = offers['is_for_sale'].astype(bool)										   
    if not os.path.exists('output'):
        os.mkdir('output')
    offers.to_csv('output/' + _offers_file + current_date + '.tsv', sep='\t', index=False)
...
def crawlLinks(type_of_offering, page_count):
    ...
    for page_n in tqdm(range(1, page_count + 1)):
        ...
        with open('C:/Users/shadow/Downloads/address_result_sale.html' if type_of_offering == _is_sale \
                else 'C:/Users/shadow/Downloads/address_result_rent.html', 'r', encoding='utf-8') as f:
            resp = f.read()
        ...
        for b in boxes:
            try:
                ...
                link =  b.findAll('a', attrs={'class': 'detail'})[0]['href']
                ...
                offers = offers.append({'link': clean_text(link),
                                        'id': id,
                                        'is_for_sale': type_of_offering == _is_sale,
                                        'type': clean_text(typ),
